...
{
    public partial class frm_Deckslab_LS : Form
    {
        IApplication iApp = null;
        public frm_Deckslab_LS(IApplication app)
        {
            InitializeComponent();
            iApp = app;
        }
        ...
        public string user_path
        {
            get
            {
                return Path.Combine(iApp.LastDesignWorkingFolder, "Deckslab Analysis [as per BS 5400]");
            }
        }
        ...
        public void Calculate_Program()
        {
            ...
            if(girder_no > 1)
            {
                ...
                double em =  e1 - ((3.8 * _b * h * (a_dash - dc) ) / (Es * Ast * (h-dc))) * ((1 - (Mq / Mg)) / 1000000000.0);
        ...
        private void Deckslab_Flexural_Cracking(List<string> list)
        {
            ...
            double em = e1 - ((3.8 * _b * _h * (_a_dash - dc)) / (_Es * Ast * (_h - dc))) * ((1 - (Mg / Mq)) / 1000000000.0);
        ...
        public void Process_Analysis()
        {
            ...
            string user_path = Path.Combine(iApp.LastDesignWorkingFolder, "Deckslab Analysis [asper BS5400]");

            if (!Directory.Exists(user_path)) Directory.CreateDirectory(user_path);
            string flPath = Path.Combine(user_path, "INPUT_DATA.TXT");
        ...
        private void Design_Of_Cantilever_Slab()
        {
            ...
            double fpll_uls = _fpll * _gf1_uls_fpll * _gf3_uls_fpll * unitwt_concrete;

            list.Add(string.Format("At ULS = fpll_uls = fpll x γf1_uls_fpll x  γf3_uls_fpll x   unitwt_concrete  "));
            list.Add(string.Format("                  = {0}  x  {1}  x  {2} x   {3} =  {4:f3}  kN/Sq.m", _fpll, _gf1_uls_fpll, _gf3_uls_fpll, unitwt_concrete, fpll_uls));
            list.Add(string.Format(""));

            double fpll_sls = _fpll * _gf1_sls_fpll * _gf3_sls_fpll * unitwt_concrete;

            list.Add(string.Format("At   SLS  =  fpll_sls = fpll x γf1_sls_fpll x  γf3_sls_fpll x   unitwt_concrete  "));
            list.Add(string.Format("                  = {0}  x  {1}  x  {2} x   {3} =  {4:f3}  kN/Sq.m", _fpll, _gf1_sls_fpll, _gf3_sls_fpll, unitwt_concrete, fpll_uls));
            list.Add(string.Format(""));
            ...
            list.Add(string.Format("Dispersal factor in Longitudinal Direction = dis_fact = {0:f4} / {1:f4} = {2:f4} ", 0.4640, _ada, dis_fact));
            list.Add(string.Format(""));
            ...
            list.Add(string.Format("MA_uls  = 67.80 x γf1_uls_fpll  x γf3_uls_fpll  +  103.69 x 0.475 x γf1_uls_fpll x γf1_uls_fpll "));
            list.Add(string.Format("        = {0:f3} x {1:f2} x {2:f2}  +  {3:f3} x {4:f3} x {1:f2} x {2:f2} ", _M, _gf1_uls_fpll, _gf3_uls_fpll, 103.69, 0.475));
        ...
        private void Cantilever_Flexural_Cracking()
        {
            ...
            //double em = e1 - ((3.8 * _b * _h * (_a_dash - dc)) / (_Es * Ast * (_h - dc))) * ((1 - (Mq / Mg)) / 1000000000.0);
            double em = e1 - ((3.8 * _b * _h * (_a_dash - dc)) / (_Es * _As * (_h - dc))) * ((1 - (Mg / Mq)) / 1000000000.0);
